...
class dataUpdater {
  ...
  subscribeBars(symbolInfo, resolution, newDataCallback, listenerGuid) { // 订阅K线数据。图表库将调用onRealtimeCallback方法以更新实时数据。
  ...
  onSubscriberDataReceived(listenerGuid, bars) { // 已收到订阅人,到这个位置我看到是更新函数，目前还是没有看到初始化函数
    ...
    if (isNewBar) {
      ...
      subscriptionRecord.listener(previousBar) // 确实是更新函数
    }

    subscriptionRecord.lastBarTime = lastBar.time
    subscriptionRecord.listener(lastBar) // 更新函数
  }
